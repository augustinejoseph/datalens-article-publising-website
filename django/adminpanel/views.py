...
from authentication.serializers import  InterestSerializer, AllUsersSerializer, AuthorSerializer
...
class AllInterestsList(APIView):    
    ...
    def get(self, request):
        try:
            ...
            return Response({"error": "Error retrieving all interests"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class CreateInterest(APIView):
    ...
    def post(self, request):
        try:
            ...
            if not interestName:
                return Response({"error": "Interest field empty"}, status=status.HTTP_400_BAD_REQUEST)
            existing_interest = Interests.objects.filter(interestName=interestName).exists()
            if existing_interest:
            ...
            if serializer.is_valid():
                ...
                response_data = {"message": "Interest created successfully", "id": instance.id, "interestName": instance.interestName}

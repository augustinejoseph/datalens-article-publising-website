    ...
    const fetchData = async () => {
      try {
        ...
        const [allUserInterests, authorDetails, allInterests, savedArticles] =
          await Promise.all([
            axios.get(
              `${BACKEND_BASE_URL}/user/user-interests-by-username/${username}`
            ),
            axios.get(`${BACKEND_BASE_URL}/user/author-details/${username}`),
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${BACKEND_BASE_URL}/user/update-user-details`,
        updatedUser
      );
      TokenRefresh();
      setEditMode(false);
